...
<?php $this->load->view('header.php');  ?>
   ...
   <div class="divm2"><a href="Transaction"><img border="0" alt="Back" src="\Gatepass\Assets\Icon\Back.jpg" width="25" height="25" title="Back"></a></div>
        ...
        <div class="div3">  
            ...
            <tr>
              <form action = "" method="GET">
                <td> Date From :</td>
                <td><input type="date" id="from_date" name="from_date" value="<?php echo $fmday; ?>" required ></td>
                <td> Date  to:</td>
                <td><input type="date" id="to_date" name="to_date" value="<?php echo $today; ?>" required ></td>
                ...
                <td> <a href="#" class="btn btn-success btn-sm" onclick="printDiv('displayData')">Print</a> &nbsp;
                     ...
                     <a href="Addgatepass"><img border="0" alt="Add" src="\Gatepass\Assets\Icon\Insert.jpg" width="25" height="25" title="Add"></a>
                </td>
              </form>
            </tr>               
            </table> 
              ...
              If ($_GET['EmployeeCode']==0)
                {
                 ...
                 $sql = "SELECT * FROM v_gatepass WHERE CompanyId=".$_SESSION["sCompanyId"]." and SiteId=".$_SESSION["sSiteId"]." and gatepassdate BETWEEN :from_date AND :to_date order by gatepassdate,gatepassno";
                }else {

                 $sql = "SELECT * FROM v_gatepass WHERE CompanyId=".$_SESSION["sCompanyId"]." and EmployeeCode='".$_GET['EmployeeCode']."' and gatepassdate BETWEEN :from_date AND :to_date order by gatepassdate,gatepassno";
                }
                  ...
                  ?>
                   ...
                   ?>
                           <tr>
                            ...
                            <form  action="Editgatepass" method="POST" enctype="multipart/form-data" >
                             <input type="text" name="GatepassId" value="<?php echo $row['GatepassId'];?>" hidden>
                             <input type="text" name="Edit" value="Edit" hidden>
                             <button name="submit" value ="Edit" title="Click to Edit"> <img src="\Gatepass\Assets\Icon\Edit.jpg" alt="Button Image" style="width: 20px; height: 20px;"> </button>
                            ...
                            <form  action="" method="GET" enctype="multipart/form-data" >
                             ...
                             <button name="submit" value ="Delete" title="Click to Delete"> <img src="\Gatepass\Assets\Icon\Delete.jpg" alt="Button Image" style="width: 20px; height: 20px;"> </button>
                           </form>                                 
          ...
          if  (isset($_GET['Delete']))
            {  
              ...
              try {
                  ...
                  // Prepare the SQL query to filter records within the date range
                //  $sql = "Delete FROM gatepass WHERE CompanyId=".$_SESSION["sCompanyId"]." and GatepassId=".$_GET['GatepassId'];
               ...
               //   $stmt->execute();
                  ...
                  // Get the user_id, record_id, and deletion_reason (these could come from a form or session)
                  $deluserid = $_SESSION["sUserId"];  // Example user ID (could be from session or user input)
                  $mGatepassId = $_GET['GatepassId'];  // ID of the record to be deleted

                  // Call the stored procedure to delete the record and log it
                  $stmt = $gconn->prepare("CALL delete_gatepass(:GatepassId, :deluserid)");
                  ...
                  echo '<script>alert("Gatepass Id : '.$_GET['GatepassId'].' removed ...")</script>';
